...
import (
	...
	"github.com/dana-team/provider-dns/apis"
	"github.com/dana-team/provider-dns/apis/v1alpha1"
	"github.com/dana-team/provider-dns/config"
	"github.com/dana-team/provider-dns/internal/clients"
	"github.com/dana-team/provider-dns/internal/controller"
	"github.com/dana-team/provider-dns/internal/features"
)

func main() {
	var (
		app               = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for DNS").DefaultEnvars()
	...
	logr := logging.NewLogrLogger(zl.WithName("provider-dns"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-dns",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add DNS APIs to scheme")
	...
	provider, err := config.GetProvider(context.Background())
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup DNS controllers")
